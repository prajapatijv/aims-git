...
Private Sub GenerateSalesReport()
    ...
    Select Case LCase(Me.Tag)
        ...
        Case LCase("rep_salsum")
            ReDim SpPrm(6) As String
            ReDim formulas(2) As String
            
            mRpt = "Sales_Ticketwise.rpt"
            ...
            SpPrm(4) = cmbStatus.ListIndex          'Status - 0:All/ 1:Active /2:Rejected
            SpPrm(5) = IIf(OperaionMode = enServer, 1, 2)
            SpPrm(6) = 0                            'Preview Enabled : Always Off : Used for Debug
            
            formulas(0) = "ReportTitle='Sales Summary Report'"
            
            
            SQL = GenReportSP("rptSalesSummary", SpPrm)
            
            gCnnMst.Execute SQL
            
        Case LCase("rep_saldet")
            ReDim SpPrm(12) As String
            ReDim formulas(2) As String
        
            If cmbGrpBy.ListIndex = 0 Then
                mRpt = "SaleDetail_Ticket.rpt"
            Else
                mRpt = "SaleDetail_Summary.rpt"
            End If
            ...
            SpPrm(11) = IIf(OperaionMode = enServer, 1, 2)
            SpPrm(12) = 0                                       'Preview Enabled : Always Off : Used for Debug
            
            formulas(0) = "ReportTitle='Sales Detail Report'"
            
            SQL = GenReportSP("rptSalesDetail", SpPrm)
            gCnnMst.Execute SQL
        ...
        Case LCase("rep_itm")
            ...
            formulas(0) = "ReportTitle='Itemwise Sales Report'"
...
Private Sub SetForm()
    ...
    Label5.Enabled = True
    cmbGrpBy.Enabled = True
    
    Select Case LCase(Me.Tag)
        Case "rep_salsum"
            Frame1.Visible = False
            lblRptHeadW.Caption = "Sales Summary Report"
            lblRptHeadB.Caption = lblRptHeadW.Caption
            
        Case "rep_saldet"
            Frame1.Visible = True
            lblRptHeadW.Caption = "Sales Detail Report"
            lblRptHeadB.Caption = lblRptHeadW.Caption
            
        Case "rep_itm"
            Frame1.Visible = True
            lblRptHeadW.Caption = "Itemwise Sales Report"
            lblRptHeadB.Caption = lblRptHeadW.Caption
            Label5.Enabled = False
            cmbGrpBy.Enabled = False
